...
class CancelOfferApiCall(ApiComponent):
    """Handles canceling a Credex offer and managing state"""
    ...
    def validate_api_call(self, value: Any) -> ValidationResult:
        ...
        try:
            ...
            logger.info(f"Canceling offer {credex_id} for member {member_id}")

            # Make API call
            url = f"cancelOffer/{member_id}/{active_account_id}/{credex_id}"
            headers = {
            ...
            # Clear offer data after successful cancellation
            self.update_component_data(data={})

            # Set component result for flow control
            action = self.state_manager.get_state_value("action", {})
            if action.get("type") == "OFFER_CANCELED":
                logger.info("Offer canceled successfully - proceeding to dashboard")
                self.update_component_data(component_result="show_dashboard")
            else:
                logger.info("Unexpected action type after canceling offer")
