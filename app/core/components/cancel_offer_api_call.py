...
class CancelOfferApiCall(Component):
    """Handles canceling a Credex offer"""
    ...
    def validate(self, value: Any) -> ValidationResult:
        ...
        # Call cancelOffer endpoint
        from core.api.credex import cancel_offer
        success, message = cancel_offer(
        ...
        if not success:
            return ValidationResult.failure(
                message=f"Failed to cancel offer: {message}",
                field="api_call",
        ...
        if not flow_data or "data" not in flow_data:
            return ValidationResult.failure(
                message="No state data after canceling offer",
                field="flow_data",
                details={"component": "cancel_offer"}
            )
