...
class CredexBotService:
                ...
                "body": {
                    "text": HOME.format(
                        ...
                        balance=BALANCE.format(
                            securedNetBalancesByDenom=current_state['member']['balanceData']['securedNetBalancesByDenom'][0] if current_state['member']['balanceData']['securedNetBalancesByDenom'] else "$0.00", 
                            totalPayables=current_state['member']['balanceData']['unsecuredBalancesInDefaultDenom']['totalPayables'], 
                            totalReceivables=current_state['member']['balanceData']['unsecuredBalancesInDefaultDenom']['totalReceivables'],
                            netPayRec=current_state['member']['balanceData']['unsecuredBalancesInDefaultDenom']['netPayRec'],
                            netCredexAssetsInDefaultDenom=current_state['member']['balanceData']['netCredexAssetsInDefaultDenom']
                        ),
    ...
    def get_file(self, url, name):
        ...
        if response.status_code == 200:
            ...
            bytesio_o = BytesIO(response.content)
            return File(bytesio_o, name=f"{name}.jpeg")
        return None
                ...
                if data:
                    ...
                    return  {
                        ...
                        "interactive": {
                            "type": "button",
                            "body": {
                                "text": ACCEPT_CREDEX.format(amount=item.get('title').split(" "),  party=item.get('description'))
                            },
                            "action": {
                                "buttons": [
                                    {
